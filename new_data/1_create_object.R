...
initial_path <- file.path("data/paad_cptac_2021")
out_path <- file.path("new_data/output")
plot_path <- file.path("new_data/plot")

## 1. Get data from nf-core piepeline ----------------------------------------------------------------------
# Load txt file with the number of reads
file = file.path(initial_path, 'data_mirna.txt')
counts_data <- read.table(file, sep = "\t", header = TRUE, row.names=1)
